...
// ⭐ FINAL ROBUST LOGIC: Using MarginV to handle multi-line expansion correctly
function framesToAss(frames, styles, videoWidth, videoHeight) {
  ...
  // --- CALCULATE SCALED MARGINS FOR STACKING ---
  // The padding value (e.g., 200px) is proportional to the 1080px reference height, 
  // so we must scale all measurements to the actual video height (playResY).
  
  const REFERENCE_HEIGHT = 1080;
  const scaleFactor = playResY / REFERENCE_HEIGHT;

  const VERTICAL_GAP = 15; // Vertical space between the two lines
  const MULTILINE_ALLOWANCE = 1.0; // Extra line allowance factor for Line 1 to wrap without colliding (1.0 = one extra line height)

  // Line 2 (BOTTOM) Margin: Scaled user padding
  const MarginV_Line2 = Math.round(paddingBottom * scaleFactor);
  
  // Line 1 (TOP) Margin: 
  // Base Margin = Line 2 Margin + Scaled Line 2 Font Size + Scaled Vertical Gap
  const baseMarginV_Line1 = MarginV_Line2 + Math.round(size2 * scaleFactor) + Math.round(VERTICAL_GAP * scaleFactor);

  // CRITICAL FIX: Add allowance for Line 1's own font size if it wraps.
  // The text box expands up from the MarginV line, so we need to push Line 1 up 
  // by its own font size, multiplied by the allowance factor.
  const scaledSize1 = size1 * scaleFactor;
  const MarginV_Line1 = Math.round(baseMarginV_Line1 + (scaledSize1 * MULTILINE_ALLOWANCE));
...
Style: STYLE2,${font2},${size2},${color2Primary},${color2Primary},&H00000000,${shadowColor},${weight2},${italic2},0,0,100,100,0,0,1,${outline},${shadow},${CENTER_ALIGNMENT_MODE},20,20,${MarginV_Line2},1
Style: STYLE1,${font1},${size1},${color1Primary},${color1Primary},&H00000000,${shadowColor},${weight1},${italic1},0,0,100,100,0,0,1,${outline},${shadow},${CENTER_ALIGNMENT_MODE},20,20,${MarginV_Line1},1
...
`;
  ...
  const events = frames.flatMap(f => {
    ...
    if (f.line1 && f.line1.trim() !== '') {
      ...
      // No \pos needed. Alignment 2 + MarginV handles position.
      lines.push(`Dialogue: 0,${assStart},${assEnd},STYLE1,,0,0,0,,${text1}`);
    }
    
    // Line 2 (BOTTOM) -> Uses STYLE2 (Lower MarginV)
    if (f.line2 && f.line2.trim() !== '') {
      const text2 = getPlainText(f.line2);
      lines.push(`Dialogue: 0,${assStart},${assEnd},STYLE2,,0,0,0,,${text2}`);
    }
