...
// -------------------------
// framesToAss() - Robust logic for all resolutions/aspect ratios
// -------------------------
function framesToAss(frames, styles = {}, videoWidth, videoHeight) {
  ...
  const scaledPaddingBottom = Math.round(basePaddingBottom * scale);
  const scaledBaseGap = Math.round(baseGapRef * scale);
  
  // --- END: Resolution Scaling Fix ---
  ...
  if (twoLinePresent) {
    // extra downward nudge proportional to *scaled* bottom font height
    const extraDownWhenTwoLines = Math.round(scaledFontSizeBottom * 0.30);
  ...
  const Y_pos_Line1 = Y_pos_Line2
    ...
    - (scaledBaseGap + scaledFontSizeBottom * extraGapFactor);
...
Style: STYLE_BOTTOM,${fontBottom},${scaledFontSizeBottom},${colorBottom},${colorBottom},&H00000000,${shadowColor},${boldBottom},${italicBottom},0,0,100,100,0,0,1,0,2,2,20,20,0,1
Style: STYLE_TOP,${fontTop},${scaledFontSizeTop},${colorTop},${colorTop},&H00000000,${shadowColor},${boldTop},${italicTop},0,0,100,100,0,0,1,0,2,2,20,20,0,1
...
`;
  ...
  const events = frames.flatMap(f => {
    ...
    if (f.line1 && f.line1.trim()) {
      ...
      // Use Math.round() for the final pixel positions
