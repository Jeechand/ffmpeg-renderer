...
function framesToAss(frames, styles, videoWidth, videoHeight) {
  ...
  // Scale font sizes to the actual video height
  const scaledSize1 = size1 * scaleFactor;
  const scaledSize2 = size2 * scaleFactor;
  const scaledGap = VERTICAL_GAP * scaleFactor;

  // Position is measured from the top. We compute a baseline for the bottom line (Line 2)
  // Important behavior change:
  // - The bottom line is anchored at (playResY - scaledPadding).
  // - If the top line exists we push the bottom line *down* slightly so the top line
  //   can be placed above it without causing the bottom line to be pushed *up*.
  const proportionalPadding = paddingBottom * scaleFactor;

  // Extra downward nudge for bottom line when top line exists (so bottom moves down)
  // The multiplier (0.3) is conservative; adjust if you want more/less downward shift.
  const extraDownWhenTopExists = frames.some(f => f.line1 && f.line1.trim() !== '') ? Math.max(0, scaledSize1 * 0.3) : 0;

  // Y-position for Line 2 (BOTTOM LINE) - Scaled based on actual video height and nudged down when needed
  const Y_pos_Line2 = playResY - proportionalPadding + extraDownWhenTopExists;
  
  // Y-position for Line 1 (TOP LINE) - Use the TOP font scaled size to position above Line 2
  // (previous bug: code used bottom font size to compute top Y; now using scaledSize1)
  const Y_pos_Line1 = Y_pos_Line2 - scaledSize1 - scaledGap;
...
`;
  ...
  const events = frames.flatMap(f => {
    ...
    if (f.line1 && f.line1.trim() !== '') {
      ...
      lines.push(`Dialogue: 0,${assStart},${assEnd},STYLE1,,0,0,0,,{\\pos(${playResX / 2},${Math.round(Y_pos_Line1)})}${text1}`);
    }
    
    // Line 2 (BOTTOM) -> Uses STYLE2 and fixed Y position
    if (f.line2 && f.line2.trim() !== '') {
      const text2 = getPlainText(f.line2);
      lines.push(`Dialogue: 0,${assStart},${assEnd},STYLE2,,0,0,0,,{\\pos(${playResX / 2},${Math.round(Y_pos_Line2)})}${text2}`);
    }
...
app.post('/render', async (req, res) => {
  ...
  try {
    ...
    let ffArgs;
    // Note: ffmpeg ASS filter wants a filename; make sure path contains no spaces or escape if needed.
    ...
    if (shouldAddWatermark) {
        if (watermarkInput) {
            ...
            // NOTE: when using two inputs, the ordering matters in the filter graph. We'll use:
            // - Input 0: Watermark image
            // - Input 1: Video
            // Scale watermark, then overlay onto video, then apply ASS
            filterComplex =
